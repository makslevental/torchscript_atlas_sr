...
class UpBlock(torch.nn.Module):
    def __init__(
        ...
        self.up_conv1 = DeconvBlock(
            num_filter, num_filter, kernel_size, stride, padding, activation, norm=False
        )
...
class D_UpBlock(torch.nn.Module):
    def __init__(
        ...
        self.conv = ConvBlock(
            num_filter * num_stages, num_filter, 1, 1, 0, activation, norm=False
        )
        self.up_conv1 = DeconvBlock(
            num_filter, num_filter, kernel_size, stride, padding, activation, norm=False
        )
...
class Upsampler(torch.nn.Module):
    def __init__(self, scale, n_feat, bn=False, act="prelu", bias=True):
        ...
        for _ in range(int(math.log(scale, 2))):
            modules.append(
                ConvBlock(n_feat, 4 * n_feat, 3, 1, 1, bias, activation=False, norm=False)
            )
...
class DBPNITER(nn.Module):
    def __init__(self, num_channels, base_filter, feat, num_stages, scale_factor):
        ...
        self.feat0 = ConvBlock(
            num_channels, feat, 3, 1, 1, activation="prelu", norm=False
        )
        self.feat1 = ConvBlock(
            feat, base_filter, 1, 1, 0, activation="prelu", norm=False
        )
        ...
        self.output_conv = ConvBlock(
            num_stages * base_filter, num_channels, 3, 1, 1, activation=False, norm=False
        )
